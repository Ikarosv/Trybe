...
function numeroQueMaisRepete(arr = [2, 3, 3, 5, 8, 2, 3]) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    arr[i] in numerosEIndices ? numerosEIndices[arr[i]].push(i) : numerosEIndices[arr[i]] = [i];
  }
  let numero = 0
  for (const key in numerosEIndices) {
    numero = numero == 0 ? key : numero
    for (const key2 in numerosEIndices) {
      numero = numerosEIndices[numero].length < numerosEIndices[key2].length ? key2 : numero
    }
  }
  return numero
}
